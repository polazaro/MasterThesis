...
## we organize the images in folders. We are going to create one folder per  ##
## label and put there all the frames with that label.                       ##
##                                                                           ##
## We are following this schedule because it is clearer to see which of the  ##
## frames belong to each of the labels and then we can create the csv with   ##
## the names of the images and the labels faster.                            ##
##                                                                           ##
###############################################################################
...
dest_path     = '/app/data/IEMOCAP_dat1/IEMOCAP_faces_2_training/4Labels/'
        ...
        for line in data:  ## Each line is an utterance
            ...
            if frames_exist.count(False) == 0:
                ...
                for j in range(init_frame, end_frame+1):
                    ...
                    frame = video_path + utt_txt[:-4] + '_{:06d}'.format(j) + '.bmp'
                    shutil.copy(frame, dest_path + 'train/' + utt[1])
                    im = Image.open(frame)
                    im = im.transpose(Image.FLIP_LEFT_RIGHT)
                    im.save(dest_path + 'train/' + utt[1] + '/' + utt_txt[:-4] + '_{:06d}'.format(j) + '_flip.bmp')
                    ok_frames += 1
                    labels.append(utt[1])
...
with open(dest_path + 'summary_train.txt', 'w') as f:   
    ...
    f.write('Number of faces: '+str(ok_frames)+'\n')
    f.write('Number of faces + flip faces: '+str(ok_frames*2)+'\n')
    f.write('Number of ok utterances: '+str(ok_utt)+'\n')
    f.write('Number of wrong utterances: '+str(wrong_utt)+'\n'+'\n')
   
    f.write('Number of faces in (for faces+flip_faces, multiply by 2):'+'\n')
        ...
        for line in data:  ## Each line is an utterance
            ...
            if frames_exist.count(False) == 0:
                ...
                for j in range(init_frame, end_frame+1):
                    ...
                    frame = video_path + utt_txt[:-4] + '_{:06d}'.format(j) + '.bmp'
                    shutil.copy(frame, dest_path + 'validation/' + utt[1])
                    im = Image.open(frame)
                    im = im.transpose(Image.FLIP_LEFT_RIGHT)
                    im.save(dest_path + 'validation/' + utt[1] + '/' + utt_txt[:-4] + '_{:06d}'.format(j) + '_flip.bmp')
                    ok_frames += 1
                    labels.append(utt[1])
    ...
    for line in data:  ## Each line is an utterance
        ...
        if frames_exist.count(False) == 0:
            ...
            for j in range(init_frame, end_frame+1):
                ...
                frame = video_path + utt_txt[:-4] + '_{:06d}'.format(j) + '.bmp'
                shutil.copy(frame, dest_path + 'test/' + utt[1])
                im = Image.open(frame)
                im = im.transpose(Image.FLIP_LEFT_RIGHT)
                im.save(dest_path + 'test/' + utt[1] + '/' + utt_txt[:-4] + '_{:06d}'.format(j) + '_flip.bmp')
                ok_frames += 1
                labels.append(utt[1])
...
''' Auxiliar code that is not needed rigth now: '''
