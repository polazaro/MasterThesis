...
class ImAudioSequence(Sequence):
    ...
    def __init__(self, df_audio, df_im, data_path_audio, data_path_im, batch_size, nb_class, rescale = False):
    ...
    def get_batch_features(self, idx):
        ...
        return [np.array([self.load_image(im) for im in self.im_list[idx * self.bsz: (1 + idx) * self.bsz]]), np.array([np.expand_dims(self.load_audio(audio), axis=1) for audio in self.audio_list[idx * self.bsz: (1 + idx) * self.bsz]])]
...
seq_train = ImAudioSequence(audio_train, im_train, '/app/data/IEMOCAP_dat1/IEMOCAP_audio_2_training_100/4Labels/train/', 
                           '/app/data/IEMOCAP_dat1/IEMOCAP_faces_2_training/4Labels/train/',
                           batch_size = batch_size, nb_class = nb_class, rescale = True)
seq_val   = ImAudioSequence(audio_val, im_val, '/app/data/IEMOCAP_dat1/IEMOCAP_audio_2_training_100/4Labels/validation/',  
                           '/app/data/IEMOCAP_dat1/IEMOCAP_faces_2_training/4Labels/validation/',
                           batch_size = batch_size, nb_class = nb_class, rescale = True)
seq_test  = ImAudioSequence(audio_test, im_test, '/app/data/IEMOCAP_dat1/IEMOCAP_audio_2_training_100/4Labels/test/',  
...
path_model = '/app/Fusion_faceraw_100/weights.000009-0.864-0.349-0.433-2.240.h5'
